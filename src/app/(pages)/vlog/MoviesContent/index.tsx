...
import { fetchMovies } from '../../../_components/FetchMovies'
import MoviesCards from '../MoviesCards'

const MoviesContent = serverUrl => {
  const [movies, setMovies] = useState<Movie[]>([]) // Tablica postów
  ...
  const getMovies = async (page: number, perPage: number) => {
    const moviesData = await fetchMovies(page, perPage, serverUrl) // Fetch 4 posty na stronę
    setMovies(moviesData.docs) // Ustawiamy posty
    setTotalPages(moviesData.totalPages) // Ustawiamy liczbę stron
  }
  ...
  useEffect(() => {
    ...
    if (savedShowAll) {
      ...
      getMovies(1, 9999) // Pobierz wszystkie posty
    } else {
      getMovies(currentPageNum, 4) // Paginacja
    }
  }, [getMovies, searchParams]) // Zaktualizuj przy zmianie query params lub "See All"
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage >= 1 && newPage <= totalPages) {
      ...
      router.push(`/vlog?page=${newPage}`) // Zaktualizuj URL
    }
  }
  ...
  const handleSeeAllToggle = () => {
    ...
    router.push('/vlog') // Zmień adres URL na /blog
  }
  ...
  const handleReturnToPagination = () => {
    setShowAll(false)
    sessionStorage.setItem('showAll', 'false') // Zapisz stan
    router.push(`/vlog?page=${currentPage}`) // Powrót do obecnej strony paginacji
  }
  ...
  return (
    <div className="pr-2 pl-2 bg-customWhite w-full flex flex-col items-center">
      <div className="w-full flex flex-col mt-10 mb-2 md:max-w-[1536px]">
        <h1 className="text-primary font-medium">Vlog</h1>
        <h2 className="text-primary-light font-normal text-2xl">Discover our new movies</h2>
      </div>
